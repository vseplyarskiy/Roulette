...
data<-read.table(file="Grant_figure_NGM_sv3",sep=" ")
...
p<-ggplot(data,aes(x = V3/2.245020e-05
,y=V2/2.245020e-05
,color=V4))+#scale_x_discrete(limits = positions)+#geom_bar()
  ...
  #geom_pointrange(aes(ymin=CI[,4]/data$V4, ymax=CI[,5]/data$V4))+
   ...
   geom_point(size = 1.2)+
  theme_bw()+
  ylab("Observed rate (de novo), log scale") +
  #ylab("Fracrion of segregated sites") +
  
  #ggtitle("CpG>ApG  (Process 9)")+
   scale_color_discrete(name="") + 
  xlab("Predicted mutation rate, log scale") +
  #ylim(c(0.9,11))+xlim(c(0.9,11))+
  #ylim(c(0,.029))+
  # xlim(0,10)+ylim(0,10)+
  scale_x_continuous(trans = "log2",breaks = c(.5,2,8,32,128))+scale_y_continuous(trans = "log2",breaks = c(.5,2,8,32,128))+
  theme(legend.text = element_text(size = 15),legend.title = element_text( size=15, face="bold"))+
  theme(plot.title = element_text(hjust = 0.5))+
  theme(  #axis.text=element_blank(),#element_text(size = 15),
