...
#	print "$a[6] $mean $a[1] $c\n";
    ...
    open (OUT,">/net/home/vseplyarsky/GNOMAD_Tufts/data/CHIPseq/data/hash/$chr\_RNU_ind_qc");
    open (IN,"gunzip -c /net/home/vseplyarsky/GNOMAD_Tufts/data/all_variants_chr$chr.csv.gz|");
    while (<IN>){
        chomp;
    my @a=split/\,/;
	next unless $fac{$a[0]};
	next unless /snv/;
	print OUT "$fac{$a[0]} $a[23] $a[17] $a[22] $a[20]\n"}
}
