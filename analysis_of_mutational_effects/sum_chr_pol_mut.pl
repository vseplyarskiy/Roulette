...
open (IN,"/net/home/vseplyarsky/GNOMAD_Tufts/data/CHIPseq/data/DHS/for_reg_all_TFBS_short_solution");
while (<IN>){
    chomp;
    my @a=split/\t/;
...
#    print "P2\n";
 ...
 #   open (OUT1,">/net/home/vseplyarsky/GNOMAD_Tufts/mut_model/validation/$chr\_rand")||die;
    open OUT, '>:gzip',  "/net/home/vseplyarsky/GNOMAD_Tufts/mut_model/for_LASSO/$chr\_rate_v5_TFBS_correction.gz"; 
 #  open (OUT,">/net/home/vseplyarsky/GNOMAD_Tufts/data/CHIPseq/data/DHS/results/$chr\_for_reg_all_TFBS");
   ...
   while (<IN>){
	...
	substr($mu2,0,1)='';
        substr($mu2,3,1)='';
	my $in="$mu2 $a[3] $fac{$pos}";
	print OUT "$a[0] $a[1] $a[2] $a[3] TFBS $sol{$in}\n";
