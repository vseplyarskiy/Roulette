...
my $sp_par=(((${$spats5{$PP}}{$int}-1)-(${$spat5{$PP}}{$int}-$pol))/(${$spats5{$PP}}{$int}-1))/$rav{$int};
...
my $o=1-$pol;
#open (OUT,">/net/home/vseplyarsky/GNOMAD_Tufts/mut_model/for_LASSO/$chr\_input");
print OUT "$p\t$o\t$in $type1\t$coeff\RT_dir$dir\t$sp_par\n";
}
   }}
